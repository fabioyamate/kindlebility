    ...
    **/
    grabArticle: function (page) {
        ...
        **/
        var node = null;
        var nodesToScore = [];
        for(var nodeIndex = 0; (allElements.length, node = allElements[nodeIndex]); nodeIndex++) {
            if (node.id == 'lemmaContent') console.log('==', ''+node, nodeIndex);
            ...
            if (node.tagName === "DIV") {
                ...
                {
                    if (node.id == 'lemmaContent') console.log('=EXPERIMENTAL', ''+node);
                    /* EXPERIMENTAL */
                    //arrix
                    ...
                    for(var i = 0, il = childNodes.length; i < il; i++) {
                        ...
                        if(childNode.nodeType == 3) { // Node.TEXT_NODE
                            ...
                            //childNode.parentNode.insertBefore(p, childNode)
        ...
        **/
        ...
        for (var pt=0; pt < nodesToScore.length; pt++) {
            var n = nodesToScore[pt]; var ya = /content/.test(n.className + n.id);
            if (ya) {console.log('yaaaaaaaaaaaaaaaaaaaaaaaaaaa', ''+n);}
            var parentNode      = nodesToScore[pt].parentNode;
            var grandParentNode = parentNode ? parentNode.parentNode : null;
            var innerText       = readability.getInnerText(nodesToScore[pt]);

            if(!parentNode || typeof(parentNode.tagName) == 'undefined') {
                console.log('nooooooooo parent?', ''+n);
                continue;
            }
            
            if (parentNode.id == 'lemmaContent') {
                console.log('======parent is lemmaContent', ''+n);
            }
            ...
            if(typeof parentNode.readability == 'undefined') {
                ...
                console.log('puuuuush parentNode', ''+parentNode);
            }
            ...
            if(grandParentNode && typeof(grandParentNode.readability) == 'undefined' && typeof(grandParentNode.tagName) != 'undefined') {
                ...
                console.log('puuuuush grandParentNode', ''+grandParentNode);
            }
            ...
            contentScore += innerText.split(reComma).length;
        ...
        **/
        ...
        console.log('========= top candiate:', topCandidate);
        /**
        ...
        **/
        if(false && readability.getInnerText(articleContent, false).length < 250) {
    ...
    cleanConditionally: function (e, tag) {
        ...
        **/
        for (var i=curTagsLength-1; i >= 0; i--) {
            ...
            else if ( readability.getCharCount(tagsList[i],reComma) < 10) { //arrix 
                /**
